...
ADAPTER_DIR="$MODEL_TRAINING_ROOT/artifacts/current/casual-lora-alpha-3/adapters/casual-alpha-3"
OUTPUT_DIR="$MODEL_TRAINING_ROOT/artifacts/current/casual-lora-alpha-3/outputs/casual-alpha-3"
RESUME_ADAPTER_FILE="${KEYVOX_RESUME_ADAPTER_FILE:-$MODEL_TRAINING_ROOT/artifacts/current/casual-lora-alpha-2/adapters/casual-alpha-2/adapters.safetensors}"
...
echo "Training Casual alpha-3 continuation adapter..."
TRAIN_ARGS=(
    --model "$MODEL" \
    --train \
    --data "$DATA_DIR" \
...
generate_case "money_five_hundred" "The renewal is twenty twenty four and the total is five hundred dollars."
generate_case "money_five_thousand_one_hundred" "The total is five thousand one hundred dollars."
