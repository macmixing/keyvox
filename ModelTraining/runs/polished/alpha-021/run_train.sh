...
DATA_DIR="$MODEL_TRAINING_ROOT/datasets/polished/alpha-021-continuation-long-numerics"
ADAPTER_DIR="$MODEL_TRAINING_ROOT/artifacts/current/polished-lora-alpha-021/adapters/polished-alpha-021"
OUTPUT_DIR="$MODEL_TRAINING_ROOT/artifacts/current/polished-lora-alpha-021/outputs/polished-alpha-021"
RESUME_ADAPTER_FILE="${KEYVOX_RESUME_ADAPTER_FILE:-$MODEL_TRAINING_ROOT/artifacts/archive/polished-lora-alpha-020/adapters/polished-alpha-020/adapters.safetensors}"
...
echo "Training Polished alpha-021 adapter..."
TRAIN_ARGS=(
    --model "$MODEL" \
    --train \
    --data "$DATA_DIR" \
...
generate_case "paragraph_distinct_meaning" "Okay, so I guess we need to test this out for a little bit.

You know, we really just need to see how paragraphs work."
generate_case "long_numeric_gauntlet" "Um hey team, I looked at the April twenty second launch notes, and there are like three things we need to clean up. Sarah and me was reviewing the checklist at eleven thirty, and we found two minor issues. I ain't worried about the build, but the screenshots still need a final pass.
