...
- Removed mistaken possessive apostrophes from plural nouns before spoken exclamation-point and question-mark commands.
...
Core now also provides portable audio, resource, linguistic, and runtime boundaries shared by Apple and Android clients while preserving platform-native behavior where it is healthy.
...
- Added portable WAV decoding, PCM resampling, and audio post-processing so non-Apple clients can supply the same speech-ready samples as Apple clients.
- Moved semantic tokenization and lexical evidence behind `KeyVoxLinguistics`, including health-routed fallback when a platform analyzer cannot provide usable evidence.
- Added portable numeric-text protection, file-extension recognition, pronunciation resources, and configurable resource-bundle resolution for non-Apple runtimes.
...
- `1.3.0` retains the embedded-number dictionary matching tracked for `1.2.9` and adds the shared portability and linguistic-routing capabilities now used by the official KeyVox clients.
