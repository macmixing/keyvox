...
- `KeyVox iOS/Core/LocalRewriteModel/LocalRewriteModelCatalog.swift`
  ...
  - Current bundled adapters are provided by `KeyVoxVibesAdapters`: `polished-alpha-023-lora.gguf` and `casual-alpha-4-lora.gguf`.
...
- `KeyVox iOS/Core/StyleRewrite/LocalStyleRewriteTextTransformer.swift`
  ...
  - Maps Polished to `polished-alpha-023`, maps Casual and Chill to `casual-alpha-4`, prewarms the selected model/adapter without generating text, sends short LoRA prompts, and logs local inference load/prefill/decode/total timing in debug builds.
