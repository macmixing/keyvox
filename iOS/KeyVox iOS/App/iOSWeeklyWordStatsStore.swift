...
@MainActor
final class iOSWeeklyWordStatsStore: ObservableObject {
    static let shared = iOSWeeklyWordStatsStore()
    ...
    init(
        ...
        installationIDGenerator: @escaping () -> String = { "ios:\(UUID().uuidString)" }
    ) {
        ...
        if let existingInstallationID = defaults.string(forKey: iOSUserDefaultsKeys.App.weeklyWordStatsInstallationID),
           !existingInstallationID.isEmpty {
            ...
            defaults.set(generatedInstallationID, forKey: iOSUserDefaultsKeys.App.weeklyWordStatsInstallationID)
        }

        if
            let data = defaults.data(forKey: iOSUserDefaultsKeys.App.weeklyWordStatsPayload),
    ...
    private func persistSnapshot() {
        do {
            ...
            defaults.set(data, forKey: iOSUserDefaultsKeys.App.weeklyWordStatsPayload)
        } catch {
            print(
                "[iOSWeeklyWordStatsStore] Failed to encode weekly word stats payload for key \(iOSUserDefaultsKeys.App.weeklyWordStatsPayload): \(error)"
            )
        }
    }
