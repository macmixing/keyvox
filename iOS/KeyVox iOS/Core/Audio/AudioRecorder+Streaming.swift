...
struct CaptureMeterUpdate: Equatable {
    let level: Float
...
final class AudioCaptureAccumulator {
    private let queue = DispatchQueue(label: "AudioRecorder.captureAccumulator")
    ...
    func snapshot() -> AudioStreamSnapshot {
        queue.sync {
            AudioStreamSnapshot(
                samples: audioData,
    ...
    func liveMetrics(sampleRate: Double) -> LiveCaptureMetrics {
        queue.sync {
    ...
    func process(
        ...
        queue.sync {
            ...
            return CaptureMeterUpdate(level: level, signalState: signalState)
        }
    }
