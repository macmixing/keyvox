...
enum DictationModelCatalog {
    ...
    nonisolated static var legacyParakeetArtifacts: [DictationModelArtifact] {
        let sharedArtifacts = parakeetArtifacts.filter { artifact in
            artifact.relativePath.hasPrefix("EncoderInt4.mlmodelc/") == false
                && artifact.relativePath.hasPrefix("JointDecisionv3.mlmodelc/") == false
        }
