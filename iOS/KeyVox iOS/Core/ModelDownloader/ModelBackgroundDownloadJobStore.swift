...
struct ModelBackgroundDownloadJobStore {
    let fileManager: FileManager
    let jobURLProvider: () -> URL?

    func load() -> ModelBackgroundDownloadJob? {
        ...
        return try? JSONDecoder().decode(ModelBackgroundDownloadJob.self, from: data)
    }

    func save(_ job: ModelBackgroundDownloadJob) throws {
