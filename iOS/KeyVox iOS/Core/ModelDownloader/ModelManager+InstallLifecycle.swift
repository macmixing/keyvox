...
extension ModelManager {
    ...
    private func applyPhaseProgress(_ phase: ModelInstallPhase, fraction: Double) async {
    ...
    private func backgroundJobStore() -> ModelBackgroundDownloadJobStore {
        ModelBackgroundDownloadJobStore(
    ...
    func performDownloadModel() async {
        ...
        do {
            ...
            let progressTracker = ModelDownloadAggregateProgress()
            ...
            async let ggmlDownload = download(ModelDownloadURLs.ggmlBase) { snapshot in
                Task {
                    await progressTracker.updateGGML(snapshot)
                    publishProgress()
                }
            }
            async let coreMLZipDownload = download(ModelDownloadURLs.coreMLZip) { snapshot in
                Task {
