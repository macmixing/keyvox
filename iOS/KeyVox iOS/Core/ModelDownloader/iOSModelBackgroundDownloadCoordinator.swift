...
final class iOSModelBackgroundDownloadCoordinator: NSObject {
    typealias StateChangeHandler = @Sendable (iOSModelBackgroundDownloadJob?) -> Void
    ...
    private let jobStore: iOSModelBackgroundDownloadJobStore
    ...
    private lazy var session: URLSession = {
        ...
        configuration.sharedContainerIdentifier = iOSSharedPaths.appGroupID
    ...
    init(
        ...
        jobStore: iOSModelBackgroundDownloadJobStore,
    ...
    func loadJob() -> iOSModelBackgroundDownloadJob? {
        jobStore.load()
    }
    ...
    func startOrResumeJob() async throws -> iOSModelBackgroundDownloadJob {
        ...
        let existingTasksByKind = Dictionary(
            uniqueKeysWithValues: tasks.compactMap { task -> (iOSModelBackgroundArtifactKind, URLSessionDownloadTask)? in
                guard let description = task.taskDescription,
                      let kind = iOSModelBackgroundArtifactKind(rawValue: description) else {
                    return nil
                }
                return (kind, task)
            }
        ...
        for kind in iOSModelBackgroundArtifactKind.allCases {
    ...
    func synchronizeWithSystemTasks() async -> iOSModelBackgroundDownloadJob? {
    ...
    private func synchronizedJob() -> iOSModelBackgroundDownloadJob {
        if let existingJob = loadJob() {
            return existingJob
        }

        return iOSModelBackgroundDownloadJob()
    }

    private func persist(_ job: iOSModelBackgroundDownloadJob) throws {
    ...
    private func ensureStagingDirectoryExists() throws {
        ...
        guard let stagingDirectoryURL = iOSSharedPaths.modelDownloadStagingDirectoryURL(fileManager: fileManager) else {
            throw CocoaError(.fileNoSuchFile)
        }
    ...
    private func stagedURL(for kind: iOSModelBackgroundArtifactKind) -> URL? {
        switch kind {
        case .ggml:
    ...
    private func notifyStateChange(with job: iOSModelBackgroundDownloadJob?) {
        stateDidChange?(job)
    }
    ...
    private func updateJob(
        ...
        mutate: (inout iOSModelBackgroundDownloadJob, inout iOSModelBackgroundArtifactState) -> Void
    ) {
        var job = loadJob() ?? iOSModelBackgroundDownloadJob()
        var artifact = job.artifactState(for: kind)
        mutate(&job, &artifact)
        job.setArtifactState(artifact, for: kind)
        try? persist(job)
...
extension iOSModelBackgroundDownloadCoordinator: URLSessionDownloadDelegate {
    func urlSession(
    ...
    func urlSession(
        ...
        guard let description = downloadTask.taskDescription,
              let kind = iOSModelBackgroundArtifactKind(rawValue: description),
