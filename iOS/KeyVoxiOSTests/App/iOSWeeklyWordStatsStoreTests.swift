...
@MainActor
struct iOSWeeklyWordStatsStoreTests {
    @Test func initPrefixesDefaultGeneratedInstallationIDForIOS() throws {
        ...
        let store = iOSWeeklyWordStatsStore(
        ...
        #expect(harness.defaults.string(forKey: iOSUserDefaultsKeys.App.weeklyWordStatsInstallationID)?.hasPrefix("ios:") == true)
    }
    ...
    @Test func initCreatesStableInstallationIDAndEmptyCurrentWeekSnapshot() throws {
        ...
        #expect(harness.defaults.string(forKey: iOSUserDefaultsKeys.App.weeklyWordStatsInstallationID) == "device-a")
        ...
        let persisted = try #require(harness.defaults.data(forKey: iOSUserDefaultsKeys.App.weeklyWordStatsPayload))
    ...
    @Test func initHydratesPersistedSnapshotForCurrentWeek() throws {
        ...
        harness.defaults.set("persisted-device", forKey: iOSUserDefaultsKeys.App.weeklyWordStatsInstallationID)
        harness.defaults.set(try JSONEncoder().encode(stored), forKey: iOSUserDefaultsKeys.App.weeklyWordStatsPayload)
    ...
    private func makeHarness(now: Date) -> WeeklyStatsHarness {
        let suiteName = "iOSWeeklyWordStatsStoreTests.\(UUID().uuidString)"
