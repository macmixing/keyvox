...
@MainActor
struct iOSiCloudSyncCoordinatorTests {
    ...
    @Test func cloudDictionarySeedsEmptyLocal() async throws {
        ...
        #expect(harness.defaults.object(forKey: iOSUserDefaultsKeys.iCloud.dictionaryLastModifiedAt) as? Date == remoteDate)
    }
    ...
    @Test func newerCloudDictionaryWins() async throws {
        ...
        harness.defaults.set(localDate, forKey: iOSUserDefaultsKeys.iCloud.dictionaryLastModifiedAt)
    ...
    @Test func lossyCloudDictionarySnapshotIsRejected() async throws {
        ...
        #expect(harness.dictionaryStore.entries.map(\.phrase) == ["Local Phrase"])
        #expect(harness.defaults.object(forKey: iOSUserDefaultsKeys.iCloud.dictionaryLastModifiedAt) as? Date == localDate)
    }
    ...
    @Test func newerCloudTriggerBindingAppliesLocally() async throws {
        ...
        let harness = try makeHarness(now: remoteDate)
        defer { harness.cleanup() }
        harness.cloudStore.seedValue(iOSAppSettingsStore.TriggerBinding.leftCommand.rawValue, forKey: KeyVoxiCloudKeys.triggerBinding)
    ...
    @Test func localTriggerBindingChangePushesCloud() async throws {
        ...
        #expect(harness.cloudStore.object(forKey: KeyVoxiCloudKeys.triggerBinding) as? String == iOSAppSettingsStore.TriggerBinding.leftControl.rawValue)
    }
    ...
    private func makeCoordinator(harness: iOSHarness) -> iOSiCloudSyncCoordinator {
    ...
    private func makeHarness(now: Date) throws -> iOSHarness {
        let suiteName = "iOSiCloudSyncCoordinatorTests.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        defaults.removePersistentDomain(forName: suiteName)

        let base = FileManager.default.temporaryDirectory
            .appendingPathComponent("iOSiCloudSyncCoordinatorTests", isDirectory: true)
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: base, withIntermediateDirectories: true)

        let settingsStore = iOSAppSettingsStore(defaults: defaults)
...
private final class InMemoryUbiquitousKeyValueStore: iOSiCloudKeyValueStoring {
