...
9. Final text is inserted via the paste service. macOS resolves following-character context, selection expansion, and direct insertion against one retained AX target. An incoming model period is stripped when supported non-quote punctuation already follows, while a differing incoming question or exclamation mark replaces that following punctuation. Failed selection expansion preserves the existing punctuation. After punctuation composition, one trailing space is appended when the result would otherwise run directly into a following letter, number, or emoji.
...
## Dictionary Hinting and Initial Entry

- Visible, persisted dictionary entries are the only entries used for provider prompts and post-processing. `KeyVoxCore` does not merge hidden app or product vocabulary.
- `DictionaryInitialEntries.keyVox` defines the single initial entry available to platform bootstrap code. KeyVox Speak and KeyVox Vibes are not initial entries.
- `KeyVoxiCloudSyncCoordinator` persists that entry only when onboarding is incomplete and no local dictionary snapshot, synchronized dictionary payload, or shared installation history exists.
- Existing installations record the shared cross-platform installation marker without changing their dictionaries. A later installation on the other platform uses that marker to avoid inserting `KeyVox` for an existing user.
- After creation, `KeyVox` behaves like every other user entry: it is visible, editable, deletable, synchronized normally, and is not recreated after deletion.
- `DictionaryHintPromptBuilder` builds prompt text only from phrases in the supplied dictionary.
- macOS `TranscriptionManager` decides whether a capture is audio-safe for dictionary hinting through `DictionaryHintPromptGate`; `DictationPipeline` also requires the supplied dictionary to be nonempty.
- `TranscriptionPostProcessor` applies the supplied dictionary entries even when provider prompt hinting is disabled for short, silent, or otherwise unsafe captures.
- Adjacent-titlecase safety checks still guard against broad prose/name rewrites, while sentence punctuation remains a real boundary for dictionary candidates followed by titlecase text in the next sentence.
...
- `Core/Services/Paste/PasteService.swift` applies shared trailing-separator policy after terminal-punctuation composition and records the latest insertion only after the complete payload, including any required trailing space, has been delivered.
- `Core/Services/Paste/Composition/PasteTerminalPunctuationCoordinator.swift` owns Mac-specific following-character lookup and guarded AX selection expansion for the same target passed directly to the injector; it returns that following character for trailing-separator composition, while punctuation classification and the preserve/replace decision remain package-owned.
- `Core/Services/Paste/MenuFallback/PasteMenuFallbackCoordinator.swift` keeps trailing spaces out of the clipboard payload and types them only after paste completion is verified. It retries that trailing-space delivery once, treats an unverified trusted paste as incomplete for this purpose, and does not authorize later latest-insertion changes when the full payload was not delivered.
